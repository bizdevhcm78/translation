...
class TranslationService:
    ...
    def _translate_single(text: str, target_lang: str, idx: int) -> TranslationResult:
        ...
        try:
            ...
            return TranslationResult(detected_source_language="unknown", text="")

    @staticmethod
    def translate_batch_meta(
        ...
        for idx, text in enumerate(cleaned_texts):
            try:
                if is_1_3B is False:
                    result = TranslationService._translate_single_meta(
                        text, target_lang, idx
                    )
                else:
                    result = TranslationService._translate_single_meta_1_3B(
                        text, target_lang, idx
                    )
                results.append(result)
    ...
    def translate_batch_mit_sua(
        ...
        for idx, text in enumerate(cleaned_texts):
            try:
                result = TranslationService._translate_single_mit_sua(
                    text, target_lang, idx
                )
    ...
    def _translate_single_meta(
        ...
        try:
            ...
            # Check if translation pair is supported
            model, tokenizer = model_manager.get_model_meta()
    ...
    @staticmethod
    def _translate_single_meta_1_3B(
        text: str, target_lang: str, idx: int
    ) -> TranslationResult:
        """Translate a single text"""
        try:
            ...
            # Check if translation pair is supported
            model, tokenizer = model_manager.get_model_meta_1_3B()
    ...
    def _translate_single_mit_sua(
        ...
        try:
            ...
            # Check if translation pair is supported
            model, tokenizer = model_manager.get_model_mit_sua()
    ...
    @staticmethod
    def translate_batch_m2m100_1_2B(
        texts: List[str], target_lang: str
    ) -> tuple[List[TranslationResult], int, int]:
        """
        Translate a batch of texts
        ...
        for idx, text in enumerate(cleaned_texts):
            try:
                result = TranslationService._translate_single_m2m100_1_2B(
                    text, target_lang, idx
                )
    ...
    @staticmethod
    def _translate_single_m2m100_1_2B(
        text: str, target_lang: str, idx: int
    ) -> TranslationResult:
        """Translate a single text"""
        try:
            ...
            # Check if translation pair is supported
            model, tokenizer = model_manager.get_model_m2m100_1_2B()
            ...
            # Preprocess text
            text = preprocess_text(text)
            tokenizer.src_lang = detected_lang_iso
            # Tokenize input
            inputs = tokenizer(
                text, return_tensors="pt", padding=True, truncation=True, max_length=512
            )
